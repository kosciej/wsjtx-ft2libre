# Differences: Our Reverse-Engineering vs Decodium Source

Comparison between our blind signal analysis (`protocol.md`) and the ground truth
from Decodium 3 source code (`protocol_from_decodium.md`).

## What We Got Right

### 4-GFSK Modulation
- **Our finding**: 4 tones, NOT 8 as claimed on ft2.it
- **Decodium confirms**: 4 tones {0,1,2,3}, Gray-coded
...
- **Decodium**: h=1.0, baud=41.667 Hz, so tone spacing = 41.667 Hz
...
- **Decodium confirms**: NSPS=288 exactly
...
- **Decodium**: Only 1 TX burst per T/R period (105 symbols = 2.52s within 3.75s cycle)
...
- **Decodium**: 103 sync+data symbols + 2 ramp = 105 total channel symbols

### Sync Pattern — Completely Wrong Approach
- **Our finding**: "No Costas arrays found", tried 7-symbol Costas arrays
- **Decodium**: Uses four **4-symbol** Costas arrays (not 7-symbol!)
...
- **Decodium**: Tones are relative to the TX frequency, not absolute.
  The base frequency is user-configurable (TxFreqSpinBox). Tone offsets
...
| Parameter | ft2libre (wrong) | Correct (Decodium) |
|-----------|-------------------|---------------------|
